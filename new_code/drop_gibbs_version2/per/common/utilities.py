...
# Modified for new test type: 10000 docs, split 1 time - cuonghn
def read_data_for_perpl(divided_data_folder):
    corpusids_part1 = list()
    corpuscts_part1 = list()
    corpusids_part2 = list()
    corpuscts_part2 = list()
    filename_part1 = '%s/data_test_part_1.txt' % (divided_data_folder)
    filename_part2 = '%s/data_test_part_2.txt' % (divided_data_folder)
    (wordids_1, wordcts_1) = read_data(filename_part1)
    (wordids_2, wordcts_2) = read_data(filename_part2)
    corpusids_part1.append(wordids_1)
    corpuscts_part1.append(wordcts_1)
...
def compute_perplexities_vb(beta, alpha, eta, max_iter, corpusids_part1,
    ...
    LD2 = 0.
    ld2_list = list()
    # modified for new test type - cuonghn
    # for k in range(5):
    for k in range(len(corpuscts_part1)):
        ld2 = vb.compute_perplexity(corpusids_part1[k], corpuscts_part1[k], corpusids_part2[k], corpuscts_part2[k])
        LD2 += ld2
        ld2_list.append(ld2)
    if len(corpuscts_part1) > 0:
        LD2 /= len(corpuscts_part1)
    return (LD2, ld2_list)
...
def write_file(i, j, beta, theta, time_e, time_m, model_folder, LD2, ld2_list,
    ...
    #    write_topic_top_words(list_tops, top_file_name)
    # write theta
    ...
    # write list top
    list_tops = list_top(beta, tops)
    write_topic_top(list_tops, top_file_name)
    write_topic_top_words(beta, tops, top_file_name)


"""
@cuonghn
For read_setting and convert string to number
"""


def write_topic_top_words(beta, tops, top_file_name):
    list_tops = list_top(beta, tops)
    write_topic_top(list_tops, top_file_name)
...
"""
@cuongnb
For read result Pw_z of obtm
"""


def read_Pw_z(filename, K, W):
    # lamda = {}
    fp = open(filename, 'r')
    i = 0
    lamda = 1 * np.random.gamma(100.0, 1.0 / 100.0, (K, W))
    while True:
        line = fp.readline()
        # check end of file
        if len(line) < 1:
            break
        terms = string.split(line)
        lamda[i] = [float(numeric_string) for numeric_string in terms]
        i += 1
    fp.close()
    return lamda
