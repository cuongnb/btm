...
def main():
    # Check input
    if len(sys.argv) != 6:
        print "Usage: python run_perplexity [dataset_name] [infer_mode] [drop_rate] [path_pw_z] [num_topics]"
        exit()
    ...
    model_name = num_topics + drop_mode + rate_name
    print model_name
    model_folder = '/'.join([dataset_dir, all_model_folder, model_name])
    divided_data_folder = dataset_dir
    infer_mode = all_mode[drop_mode]
    run_em(train_file, setting_file, model_folder, divided_data_folder, infer_mode, drop_rate, path_pw_z, num_topics)


def run_em(train_file, setting_file, model_folder, divided_data_folder, infer_mode, drop_rate, path_pw_z, num_topics):
    tops = 20
    ...
    ddict['num_topics'] = int(num_topics)
    ddict['alpha'] = 50.0/float(num_topics)
    print ddict['alpha']
    # write settings
	...
	if n%10 !=0:
        ...
        (LD2, ld2_list) = utilities.compute_perplexities_vb(beta, ddict['alpha'], ddict['eta'],
                                                            ...
                                                            corpuscts_part2)
        list_tops = utilities.list_top(beta, tops)
	#list_tops = []
        batch_no = 290
        theta = []
        time_e = 0
	...
	iter_loop=1
        # write files
        utilities.write_file(iter_loop, n, beta, theta, time_e, time_m, model_folder, LD2, ld2_list,
                             list_tops, tops)
