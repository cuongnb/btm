# -*- coding: utf-8 -*-
"""
Created on Tue Feb  3 16:00:00 2015
...
def main():
    ...
    for m in range(2, len(sys.argv)):
        ...
        filename_mean_LCP = '%s/mean-LCP-coherence-top20.csv' % (model)
        fmean_LCP = open(filename_mean_LCP, 'w')
        filename_median_LCP = '%s/median-LCP-coherence-top20.csv' % (model)
        fmedian_LCP = open(filename_median_LCP, 'w')
        filename_coh_LCP = '%s/coherence-LCP-top20.csv' % (model)
