# -*- coding: utf-8 -*-
"""
Created on Tue Mar 24 14:21:41 2015
...
def main():
    if (len(sys.argv) < 3):
        print 'usage: python NPMI.py <data-file> <list-of-model-folders>'
        sys.exit(1)
    ...
    for m in range(2, len(sys.argv)):
        ...
        for i in range(I):
            K = int(J / 10)
            for j in range(K):
                filter_topics(model, i, j*10 + 9, filtered_tops)
    filtered_tops.sort()
    ...
    for m in range(2, len(sys.argv)):
        ...
        for i in range(I):
            K = int(J/10)
            for j in range(K):
                print 'loop %d minibatch %d' % (i + 1, j*10 + 10)
                # Read the learned model
                print '\t reading model...'
                list_top = read_topics(model, i, j*10 + 9)
                K = list_top.shape[0]
                T = list_top.shape[1]
                    ...
                    for ii in range(1, T):
                        for jj in range(0, ii):
                            if (duv[index[ii]][index[jj]]) != 0 and (df[index[ii]] > 0) and (df[index[jj]] > 0):
                                total_NPMI += -1. + (np.log(df[index[ii]] * df[index[jj]]) - 2 * logN) / (
                                    np.log(duv[index[ii]][index[jj]]) - logN)
